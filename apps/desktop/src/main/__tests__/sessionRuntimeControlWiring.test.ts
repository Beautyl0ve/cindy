...
describe('session runtime control wiring', () => {
  ...
  it('rejects terminal tasks inside the shared route lock before runtime mutations', () => {
    ...
    expect(setModel).toContain('return internalOptions.lockAlreadyHeld');
    expect(setModel).toContain('? applyLocked()');
    expect(setModel).toContain(': withSendToSessionLock(sessionId, applyLocked);');
    expect(registerSource).toMatch(
      /withSendToSessionLock\(sessionId, \(\) =>\s*runPlanReviewModelApprovalTransaction\(/,
    );
    expect(registerSource).toContain(
