...
5. **Worker attention 必须按 reason / request 分离（状态：不变量）**
   renderer 进程级 attention store 同时承载两种正交 reason：`done` 是 edge-trigger 的完成未读，Worker 状态跳变进 `done` 才标记、正在查看时只确认这一个 reason；`permission(requestId)` 是从该 Worker session 的 `makerChatStore.pendingPermission` 投影出的 live 阻塞态，只能由同 requestId 的 resolve / dismiss 清除。切换 Worker 不得把权限当作已读清掉，也不得让同一轮 done 重新变未读；一个 Worker 的权限解决不得清另一个 Worker，权限与 done 并存时也只能分别清理。Worker session / team 删除才按 Worker id 清掉全部 reason。支持 reason 色彩语义的独立入口中，done 使用完成未读色，permission 使用既有 awaiting 色；`RolePillDropdown` 继续通过 Map 的 `.has(workerId)` 兼容原有通用 attention 点，不在状态投影改动中重写该组件。实现指针：`useOrcaWorkerAttentionWatcher.ts` 的 `computeWorkerAttentionUpdates`，`RolePillDropdown.tsx` 的 selected Worker clear effect，`workerAttentionStore.ts` 的 reason map，`OrcaWorkersAttentionIcon.tsx` 的 reason-aware indicator。

   Agent Island 对 Orca Worker 仍默认降噪：普通 event、ask / plan / plugin interaction 都不得放行；只有无法自行推进的 `permission` interaction 可复用既有安全摘要、owner-scoped requestId cleanup 与 session focus 链路。Worker 卡片只能跳回应用内的完整权限 UI，不得在缺少真实命令的安全摘要上直接批准；这个窄口不得携带 provider-authored display fields、suggestions、原始 tool input / metadata，也不得演变成 Worker 全量通知开关。实现指针：`agent-island/notificationPolicy.ts`、`maker-ipc/register.ts` 与 `agent-island/service.ts`。
